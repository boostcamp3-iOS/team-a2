...
class FilterViewController: UIViewController {
    ...
    let sectionArray: [FilterSection] = [.recent, .filter, .keywords, .entries]
    let filters: [(name: String, image: String, count: Int)] = [("즐겨찾기", "filterHeart", 0),
                                                                ("태그", "filterTag", 0), ("장소", "filterPlace", 0), ("연도", "filterYear", 0), ("날씨", "filterWeather", 0), ("작성 디바이스", "filterDevice", 0)]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .recent:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: filterSectionItem.identifier,
                                                           for: indexPath) as? FilterTableCell else { preconditionFailure("Cell Error") }
            return cell
        case .filter:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: filterSectionItem.identifier,
                                                           for: indexPath) as? FilterTableCell else { preconditionFailure("Cell Error") }
        ...
        case .keywords:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: filterSectionItem.identifier,
                                                           for: indexPath) as? SearchedKeywordCell else { preconditionFailure("Cell Error") }
            cell.bind(keyword: searchBar.text, count: matchedEntriesData.count)
            return cell
        case .entries:
            guard let cell = tableView.dequeueReusableCell(withIdentifier: filterSectionItem.identifier,
                                                           for: indexPath) as? MatchingEntriesCell else { preconditionFailure("Cell Error") }
                // 검색 키워드와 일치하는 단어 색상 변경
                let matchedRow = matchedEntriesData[indexPath.row]
                let range = (matchedRow as NSString).range(of: searchBar.text ?? "")
...
//        filterTableView.register(.self, forCellReuseIdentifier: FilterSection.recent.identifier)
        ...
        filterTableView.register(FilterTableCell.self,
