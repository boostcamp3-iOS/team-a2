...
class SideMenuViewController: UIViewController {
    override func viewDidLoad() {
        ...
        self.view.addGestureRecognizer(UISwipeGestureRecognizer(target: self, action: #selector(dismissFromFilterViewController)))
    ...
    @objc func dismissFromFilterViewController(_ sender: UISwipeGestureRecognizer) {
        print("스와이프")
    ...
    //FIXME: 이렇게 해도 되는지 모르겠다
    let mainCellIcon = ["sideMenuFilter", "sideMenuCalendar"]
    let mainCellLabel = ["Filter", "On this Day"]
...
extension SideMenuViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            let cell = tableView.dequeueReusableCell(withIdentifier: "mainCellId", for: indexPath) as? SideMenuMainCell
            cell?.mainIcon.image = UIImage.init(named: mainCellIcon[indexPath.row])
            cell?.mainLabel.text = mainCellLabel[indexPath.row]
            return cell!
        case 1:
            let cell = tableView.dequeueReusableCell(withIdentifier: "listCellId", for: indexPath) as? SideMenuJournalListCell
            cell?.journalTitleLabel.text = journalTitle[indexPath.row]
            cell?.journalCountLabel.text = journalCount[indexPath.row]
            return cell!
        case 2:
            let cell = tableView.dequeueReusableCell(withIdentifier: "addCellId", for: indexPath) as? SideMenuJournalAddCell
            return cell!
        default:
            let cell = tableView.dequeueReusableCell(withIdentifier: "editCellId", for: indexPath) as? SideMenuEditCell
...
// MARK: 사이드 뷰컨으로 이동하기: 익스텐션에 tapToSideMenu를 넣어서 다른 뷰컨트롤러에 addGestureRecognizer만 넣고 사용할 수 있따!
extension UIViewController {

//    self.view.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(tapToSideMenu)))
